...
| Rumpus Room (`prefer_local_intents` stays true — Tom's ruling) | `script.voice_rumpus_room_{bright,dim,color_toggle}`, lamp, `climate.rumpus_room_breeze` | Verified by text three times and heard by Tom. Starts at **50 %** (his choice), saves whatever level the KEFs report (on 2026-09-19 that was 25 % from 13:44 on, **not** the assumed 92 % PC level: 25 % is what the speakers set themselves each time they return to the PC input — 92 % before that, 50 % during voice music; issue #159) in `input_number.rumpus_room_kef_saved_volume`, restores it 10 min after music stops (`automation.rumpus_room_kefs_restore_volume_after_voice_music`). Add-to-queue / play-next and the 10-minute restore were verified on the KEFs on 2026-09-19 evening (rollout plan *Phase 3*). **Not verified:** a request while the KEFs are fully asleep (they wake at their own 20 %; the script waits up to 15 s for `playing` and sets 50 % again). Best tested first thing, before anyone has used that PC. |
