...
| Movie Room | `script.voice_movie_room_{bright,dim,red_night_mode,ambient_scene,color_toggle}`, recessed + ambient lights, LG TV, Shield, `climate.movie_room_breeze` | Routing verified by text-as-satellite many times on 2026-09-19 (requests land on the Port and play). **It is not a silent test bed** (corrected 2026-09-20 from the AVR's history; Tom confirmed the design): the Sony STR-AZ5000ES is "Works with Sonos" — 2.3 s after the Port starts playing the AVR powers on, its volume jumps to track the Port (Port 67 % → AVR 0.69) and a second later its input switches to `Sonos`; it stays on afterwards. So those text tests played out loud, and **no `pre_actions` AVR step is needed**. The Port is the preferred path for this room (Tom: it groups with the rest of the house; the AVR's own Music Assistant/Chromecast player `media_player.str_az5000es_2210_2` does not and has no area). Open question is only the Port's reliability: it went `unavailable` twice on 2026-09-19 (20:38, 20:41 — during the SonosNet-off / port re-enable work) and has been continuously available since 20:42 that evening. Of the four AVR registrations, `media_player.str_az5000es` (songpal) is the controllable one. Not yet heard by Tom by voice. |
