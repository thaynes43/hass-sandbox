...
| | Dev | Production |
|---|---|---|
| **Location** | `hass-sandbox/appdaemon/` | Docker image `ghcr.io/thaynes43/appdaemon` |
| **AppDaemon** | Run locally with `appdaemon -c appdaemon` | Runs in Kubernetes on cluster |
| **UI** | N/A | https://appdaemon.haynesops.com/ |

Everything we do in this repo considers `appdaemon/` as the **development environment**. AppDaemon runs in Kubernetes as a custom Docker image. Iterate on apps in `appdaemon/`, then merge to `main` to deploy.
...
- Processes `apps-prod.yaml` → `apps.yaml` (strips `disable: true` and `debug_preserve_run_dirs`)
- Stages app code and providers in the image
- At runtime, entrypoint copies code to `/conf/apps/` (writable emptyDir)

### What is NOT in the image

- `appdaemon.yaml` and `secrets.yaml` — come from Kubernetes Secret mounts
- Test files, dev configs, `.venv`, `__pycache__` — excluded by `.dockerignore`

### After merge

- Verify GitHub Actions build succeeded
- Check Flux reconciliation and pod status
- Verify at https://appdaemon.haynesops.com/ and check logs

### Scope communication

When you edit `appdaemon/**`:
...
├── appdaemon.yaml            # Local dev config (committed; never in image)
├── secrets.yaml              # Local dev secrets (.gitignored; never in image)
├── requirements.txt
└── apps/
    ├── apps-prod.yaml     # Production app list (all disable: true). Docker build strips disable and writes apps.yaml.
...
- `agent-docs/roadmap.md` — AppDaemon tooling direction
- `agent-docs/appdaemon-setup.md` — Dev setup and deploy summary
