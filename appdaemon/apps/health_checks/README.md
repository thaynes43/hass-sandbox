...
health_check_controller:
  ...
  # The HEADLESS service on purpose: it resolves to every replica, and the
  # client fans each post out to all of them (Alertmanager HA pairs do not
  # replicate the alert set, so a ClusterIP left both replicas paging).
  ...
  # Improvement hold — a resolve or de-escalation of a FIRING alert must be
  # sustained this long before it is applied. Stops flap page-storms (the
  # Pushover receiver has send_resolved: true, so every resolve is a page).
  alert_improve_hold_s: 900      # default 0 (act immediately); 900 in production
