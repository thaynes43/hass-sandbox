...
from health_checks.fan_health_checker.fan_health_checker import (
    FanHealthChecker,
    REPAIR_FAILED,
    REPAIR_IDLE,
    REPAIR_IN_PROGRESS,
...
def _startup(app: FanHealthChecker, mock_prov: MagicMock | None = None) -> None:
    ...
    app.initialize()
    with patch(
        "health_checks.fan_health_checker.fan_health_checker.HAProvisioner",
...
class TestChecks:
    ...
    def test_fan_ping_ok(self):
        ...
        _init_only(app)
        with patch(
            "health_checks.fan_health_checker.fan_health_checker.ping_check",
