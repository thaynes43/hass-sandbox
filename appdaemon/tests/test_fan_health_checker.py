...
class TestLifecycle:
    ...
    def _turn_on_calls(app):
        return [
            ...
            and c.kwargs.get("entity_id") == "input_boolean.fans_health_auto_repair"
        ]
...
TOGGLE_HELPER = "input_boolean.fans_health_auto_repair"
DELAY_HELPER = "input_number.fans_health_auto_repair_delay"


def _helper_states(app: FanHealthChecker, states: Dict[str, Any]) -> None:
    ...
    a test can flip a helper from unreadable to readable mid-run — and an
    unset key reads back as None, which is also what the repair script's
    "is it busy" probe wants (not busy).
    """
...
def _drive(app: FanHealthChecker, captured: list) -> None:
    """Await the captured repair coroutine without the real poll sleeps."""
    with patch.object(fhc_module, "REPAIR_POLL_INTERVAL_S", 0):
        _run(captured.pop(0))


def _script_calls(app: FanHealthChecker) -> list:
    return [
        c for c in app.call_service.call_args_list
        if c.args and c.args[0] == "script/turn_on"
    ]


def _long_down(app: FanHealthChecker, fan_name: str = "Pink Room") -> None:
    """Put *fan_name* well past any plausible grace deadline."""
    app._fan_unhealthy_since[fan_name] = (
        datetime.datetime.now() - datetime.timedelta(minutes=30)
    )
...
class TestUnreadableToggleKeepsTheDefault:
    ...
    def test_unreadable_toggle_keeps_the_cached_value(self, raw):
        app = _make_app({"auto_repair_enabled_default": True})
        _init_only(app)
        _helper_states(app, {TOGGLE_HELPER: raw, DELAY_HELPER: "5"})
    ...
    def test_a_real_off_still_disables(self):
        app = _make_app({"auto_repair_enabled_default": True})
        _init_only(app)
        _helper_states(app, {TOGGLE_HELPER: "off", DELAY_HELPER: "5"})

        _run(app._refresh_auto_repair_config())

        assert app._cached_auto_repair_enabled is False

    def test_unreadable_toggle_still_runs_the_repair_script(self):
        """The end-to-end consequence: the fan really is power-cycled."""
        app = _make_app({"auto_repair_enabled_default": True})
        _init_only(app)
        ...
        calls = _script_calls(app)
        assert len(calls) == 1
        assert calls[0].kwargs["entity_id"] == "script.zen32_hard_reset"
        assert (
            calls[0].kwargs["variables"]["unavailable_fan_entity"]
            == "fan.pink_room_fan_fan"
        )
        assert app._fan_repair_states["Pink Room"]["status"] == REPAIR_SUCCESS
...
class TestRepairConfigCommandUpdatesTheCache:
    ...
    off from the card would see it off in the card and in HA, and the fan
    would still get power-cycled.
    """

    def _cmd(self, app: FanHealthChecker, **payload: Any) -> None:
        app._on_repair_command(
            "health_check_repair_fans",
...
class TestUnreadableToggleLogging:
    def test_unreadable_warns_once_then_recovers_at_info(self):
        ...
        app = _make_app()
        _init_only(app)
        _helper_states(app, {TOGGLE_HELPER: None, DELAY_HELPER: "5"})
...
class TestRepairConfigWriteFailures:
    ...
    def test_unparseable_delay_does_not_skip_the_rest_of_the_command(self):
        ...
        assert not [
            c for c in app.call_service.call_args_list
            if c.args and c.args[0] == "input_number/set_value"
        ]


class TestDelayIsClamped:
    """A delay of 0 collapses the per-fan grace gate — a hard safety limit.
    ...
    nothing can correct it. A zero grace means a fan that blips
    entity-unavailable for one cycle is power-cycled on the spot.
    """
    ...
    def test_a_zero_delay_cannot_collapse_the_grace_period(self):
        """The end-to-end consequence: no script call on the first bad cycle."""
        app = _make_app({"auto_repair_enabled_default": True})
        _init_only(app)
        _helper_states(app, {TOGGLE_HELPER: None, DELAY_HELPER: None})
        _run(app._refresh_auto_repair_config())
    ...
    def test_the_clamp_warning_is_once_per_episode_not_per_cycle(self):
        ...
        app.entity_states[DELAY_HELPER] = "5"
        _run(app._refresh_auto_repair_config())
        assert len(_clamp_warnings()) == 1
